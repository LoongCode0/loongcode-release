...
- **记忆管理** —— 专属的「设置 → 记忆」面板，无需离开应用即可列出、预览、编辑、新建、删除各 CLI 的持久化指令文件：Claude 的 `CLAUDE.md`、Codex / OpenCode 的 `AGENTS.md`（均含全局与项目级两个层级），以及 Claude 的**自动记忆（auto-memory）**主题文件，以可搜索、可自由命名的集合形式呈现。
- **Hooks（钩子）设置** —— 在 UI 里直接查看、新增、编辑、删除 Claude 与 Codex 的生命周期钩子（工具调用前后、会话开始 / 结束等）—— 覆盖 Claude 的用户 / 项目共享 / 项目本地三层 `settings.json` 与 Codex 的 `config.toml`，已知字段提供结构化表单（无法识别的字段回退为原始文本），并附带插件或托管策略贡献的钩子的只读汇总视图。
...
- **用量统计（Usage）** —— 直观查看消耗；通过 Claude 或 Codex OAuth 登录后，侧边栏与该供应商的登录卡片上会显示实时用量徽标（会话 / 每周百分比，每 5 分钟刷新一次），Codex 用量需通过实验性开关手动开启。

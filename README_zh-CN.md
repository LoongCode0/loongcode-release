...
- **终端 TUI 模式（分栏级）** —— 每个分栏都可以在图形对话与内嵌终端之间自由切换，终端里驱动的是**与原生交互式 CLI TUI 完全相同的会话**（Claude / Codex / OpenCode 均支持）：终端从对话中断处精确续接，切回对话时终端内新产生的轮次也会被回读进聊天视图。终端实例统一池化保活，分栏的重新拆分 / 重新挂载都不会销毁它（不丢 scrollback、不闪黑屏）；会话被终端占用期间会出现一条持有态横幅（「进入终端 / 立即结束」），并禁用发送 / 分叉 / 回滚等入口，令两种模式互斥不冲突，静默无操作 10 秒后自动排水、结束持有。
- **跨对话深度搜索** —— `Ctrl+Shift+F`（或搜索框入口）打开**覆盖全部任务对话历史的全文检索**（Claude、Codex、Kimi 会话一视同仁，归档任务可按需纳入）：输入防抖、结果按任务分组、多关键词高亮并支持键盘导航，点击命中项直达原消息，并短暂高亮定位。
...
- **交互式提问（AskUserQuestion）** —— 在 UI 里直接点选选项；悬停选项可在分屏中预览详情（点击可锁定预览，窄面板下降级为堆叠布局），支持「其他」自定义输入，以及**「先讨论」入口**（作答前可与 AI 展开自由对话，对齐官方的 Chat-about-this 流程）。
- **AI 消息工具栏**、对话内链接用系统浏览器打开、任务状态与未读指示。

### 输入（Composer）
- **斜杠命令面板**（`/command`，内置 20 个 Claude 命令 / 技能的目录，说明文案跟随界面语言）与 **@ 文件提及面板**、内联文件 chip。
- **图片输入**、**每任务独立草稿**、**模型 / 推理强度选择器**（Claude 登录内置 **Fable** / Opus / Sonnet / Haiku 档位；Codex 登录内置 **Codex OAuth** 原生档位与 minimal～xhigh 推理强度；OpenCode 复用应用内 **OpenAI 兼容供应商** 并可为每个任务选择思考深度；Kimi 登录内置经 Kimi OAuth 提供的 **K2.7 / K2.7 Highspeed / K3 / K3 [1m]** 原生档位，同样可复用应用内供应商；模型下拉按当前 CLI 家族过滤）、可自定义的输入工具栏。
...
### 配置与扩展
- **Skills（技能）/ MCP 服务器 / Plugins（插件）/ 子智能体（Subagents）** 的可视化管理 —— 覆盖 **Claude / Codex / OpenCode / Kimi** 四个 CLI 家族（OpenCode 天生共享 Claude 技能根、经托管 `opencode.json` 管理 MCP 与技能 / 子智能体 / 斜杠命令的启停；Kimi 拥有独立的技能扫描、用户级与工作区级 MCP `mcp.json` 增删改查（改动自动重启子进程）、**从 marketplace 安装插件**（装卸启停即时生效），子智能体经 `AGENTS.md` 管理）；子智能体支持**用户 / 项目 / 插件**三作用域的查看、新建、编辑、删除与启停（插件提供者只读）。
- **记忆管理** —— 专属的「设置 → 记忆」面板，无需离开应用即可列出、预览、编辑、新建、删除各 CLI 的持久化指令文件：Claude 的 `CLAUDE.md`、Codex / OpenCode / Kimi 的 `AGENTS.md`（均含全局与项目级两个层级），以及 Claude 的**自动记忆（auto-memory）**主题文件，以可搜索、可自由命名的集合形式呈现。
- **Hooks（钩子）设置** —— 在 UI 里直接查看、新增、编辑、删除 Claude、Codex 与 Kimi 的生命周期钩子（工具调用前后、会话开始 / 结束等）—— 覆盖 Claude 的用户 / 项目共享 / 项目本地三层 `settings.json` 与 Codex / Kimi 的 `config.toml`，已知字段提供结构化表单（无法识别的字段回退为原始文本），并附带插件或托管策略贡献的钩子的只读汇总视图。
- **Model Providers（模型供应商）** 配置，自由切换后端 —— 常见模型内置**上下文窗口出厂默认值**，新增模型时输入框占位符直接给出推荐值（留空也有合理默认）。每个供应商可独立设置**网络代理**（跟随系统 / 自定义 / 关闭），由 CLI 登录支撑的供应商（例如 Codex OAuth 中转）还提供应用内**登录 / 状态 / 登出**流程，登录命令可编辑，并支持一键刷新模型目录。
- **依赖管理 + 运行时版本管理** —— 依赖列表从静态目录即时渲染（每项的安装状态在后台懒检测），按必须 / 可选分层呈现；**必备依赖 Claude Code CLI 与 Git 支持应用内一键安装**（Windows 直接装好，macOS 的 Git 走系统 Xcode 命令行工具引导），**OpenAI Codex 与 OpenCode CLI 亦可应用内一键安装**（三家 AI CLI 至少装其一即可）；对 `uv` / `pnpm`（及 `bun`）可列出 / 安装 / 卸载 / 切换多版本，并支持一键兜底安装。
...
- **用量统计（Usage）** —— 直观查看消耗；通过 Claude 或 Codex OAuth 登录后，侧边栏与该供应商的登录卡片上会显示实时用量徽标（会话 / 每周百分比，每 5 分钟刷新一次），Codex 用量需通过实验性开关手动开启。**账号用量全局开关**（设置 → 通用）可一键关闭整个功能：侧边栏徽标彻底隐藏、后台轮询与手动刷新一并静默、供应商登录卡的用量子块转为禁用提示——即时生效，无需重启。macOS 上，后台检查用量 / OAuth 登录状态也不再触发系统「访问其他 App 数据」的权限弹窗。

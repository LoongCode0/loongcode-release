...
![version](https://img.shields.io/badge/version-0.10.0-6d5efc)
...
### Input (Composer)
- **Slash command palette** (`/command`, a built-in catalog of 20 Claude commands/skills whose descriptions follow your interface language) plus an **@ file-mention palette** and inline file chips.
- **Image input**, **per-task independent drafts**, a **model / reasoning-effort picker** (Claude login ships with built-in **Fable** / Opus / Sonnet / Haiku tiers; Codex login ships with native **Codex OAuth** tiers and minimal–xhigh reasoning effort; OpenCode reuses the app's built-in **OpenAI-compatible providers** and lets you pick a thinking depth per task; Kimi login ships with native **K2.7 / K2.7 Highspeed / K3 / K3 [1m]** tiers via Kimi OAuth and can reuse the app's providers as well; the model dropdown filters by the current CLI family), and a customizable input toolbar.
...
- **Visual management of Skills / MCP servers / Plugins / Sub-agents** — Covers all four CLI families, **Claude / Codex / OpenCode / Kimi** (OpenCode natively shares Claude's skills root, and manages MCP plus enabling/disabling skills/sub-agents/slash commands via a managed `opencode.json`; Kimi gets its own skills scan, user- and workspace-level MCP `mcp.json` CRUD with automatic subprocess respawn on change, **plugin install from a marketplace** with install/uninstall/enable/disable taking effect immediately, and sub-agents managed via `AGENTS.md`); sub-agents support viewing, creating, editing, deleting, and enabling/disabling across **user / project / plugin** scopes (plugin-provided ones are read-only).
- **Memory management** — A dedicated Settings → Memory panel lists, previews, edits, creates, and deletes each CLI's persistent instruction files without leaving the app: Claude's `CLAUDE.md` and Codex/OpenCode/Kimi's `AGENTS.md` (both global and per-project), plus Claude's **auto-memory** topic files, shown as a searchable, freely-named collection.
- **Hooks settings** — View, add, edit, and delete Claude, Codex, and Kimi lifecycle hooks (before/after tool calls, session start/end, and more) right in the UI — covering Claude's user / project-shared / project-local `settings.json` layers and Codex's / Kimi's `config.toml`, with a structured form for known fields (raw-text fallback for anything unrecognized) plus a read-only aggregated view of hooks contributed by plugins or managed policy.
- **Model Providers** configuration, freely switch backends — common models ship with **factory-default context window sizes**; when adding a model, the input placeholder shows a recommended value directly (a sensible default even if left blank). Each provider can independently set its **network proxy** (follow-system / custom / off), and providers backed by a CLI login (e.g. a Codex OAuth relay) get an in-app **login / status / logout** flow with an editable login command and a one-click model-catalog refresh.

...
- **Session sharing** — Pick messages **in place right inside the conversation** (checkbox selection mode with select-all, a live count, and a validity period), then mint a share link in one click; a management dialog (entered via a bottom-left badge) lists everything you've shared with copy / open / revoke / clear-all actions. Links are served by a **self-hostable share server**, configured (server address + publish key) in a dedicated Settings → Sharing section.

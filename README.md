...
![version](https://img.shields.io/badge/version-0.10.4-6d5efc)
...
- **Group chat / multi-member collaboration** — Build a team of AI agents from your member library (Claude / Codex / OpenCode / Kimi / Qoder members wearing their own model, thinking-depth, and icon per family) and converse in a single shared conversation: the orchestrator serializes turns in chains (serial within a chain, parallel across chains, globally serial per member), supports **fine-grained per-member emergency stop** or a **full-group stop** (tombstone swallows late events), and persists everything across restarts (three-table schema, hidden-task exemption, layer-2 brain-swap recovery). Messages render as streaming segmented conversations — thinking goes to a dedicated field (no thinking-noise in the main text), bubbles collapse per-member with tool-group batch expansion, in-bubble @member chips render inline, and segments interleave in arrival order. The Composer integrates with a two-section @-panel (member / @all). A global **Settings -> Experimental** section houses the group-chat toggle (off by default).

...
![version](https://img.shields.io/badge/version-0.9.10-6d5efc)
...
- **Memory management** — A dedicated Settings → Memory panel lists, previews, edits, creates, and deletes each CLI's persistent instruction files without leaving the app: Claude's `CLAUDE.md` and Codex/OpenCode's `AGENTS.md` (both global and per-project), plus Claude's **auto-memory** topic files, shown as a searchable, freely-named collection.
- **Hooks settings** — View, add, edit, and delete Claude and Codex lifecycle hooks (before/after tool calls, session start/end, and more) right in the UI — covering Claude's user / project-shared / project-local `settings.json` layers and Codex's `config.toml`, with a structured form for known fields (raw-text fallback for anything unrecognized) plus a read-only aggregated view of hooks contributed by plugins or managed policy.
